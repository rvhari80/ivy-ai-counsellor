...
HOT_THRESHOLD = settings.HOT_LEAD_SCORE_THRESHOLD
COOLDOWN_MIN = 30  # Default cooldown minutes
...
async def send_email_alert(to_email: str, subject: str, html: str) -> bool:
    ...
    try:
        message = Mail(
            from_email=settings.ADMIN_EMAIL or "noreply@ivyoverseas.com",
            to_emails=to_email,
            subject=subject,
            html_content=html,
        )
...
async def notify_hot_lead(session_id: str, result: IntentResult) -> None:
    ...
    # Send to admin email (could be made configurable for counsellor)
    await asyncio.gather(
        send_whatsapp_alert(settings.WHATSAPP_PHONE_NUMBER, body) if settings.WHATSAPP_API_KEY else asyncio.sleep(0),
        send_email_alert(settings.ADMIN_EMAIL, "HOT LEAD - IVY AI Counsellor", html),
    )
